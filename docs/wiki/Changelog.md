...
> Includes interim **1.1.3–1.1.9** work (those tags were internal-only; notes live here).

- **Weekly ops digest** — Insights → Digest (+ Overview teaser) summarizes the week: grade, crashes, disk change, MSPT trend, mod churn, and one “do this next” action; stays local in ops-cache; no email/webhooks
- **Jar drift + client-only Issues** — same name+version with a different checksum → Jar drift; high-confidence client-only jars show as Issues
- **Watchdog / OOM Killed** — next boot can show a **Killed** crash when the previous session was force-killed with no Minecraft crash report (OOM vs panel timeout fix copy)
- **Restart hygiene** — Overview suggests a maintenance restart when uptime + GC/heap look worse, plus the next quiet window from Schedule; never auto-restarts
- **Dashboard timezone** — Settings → Timezone shows Schedule / quiet-window times in your zone; stored data stays UTC
- **Silent script / datapack failures** — KubeJS, CraftTweaker, datapack JSON, and `/reload` errors that never crash become Issues
- **World pressure** — continuous entity/chunk census by dimension; item-storm / mob-spike; Insights → World; Issues `WORLD_PRESSURE` (never auto-cleans). 1.1.8 pack-pin stayed deferred
...
- **Soft hang / freeze** — critical Issue when the server tick stalls; optional hang dump; likely cause + suspect-mod hint when available
...
- **Logs viewer** — `latest.log` / `debug.log` / `.log.gz` / crash reports with level filters, search, problems-only, copy/download
- **Activity incident story** — stitched lag / crash / pack / backup narrative; Overview teaser jumps to Activity
- **Theme + accent** — rail Customize + Settings → Appearance; per account
- **Look** — signal-blue accent, tighter corners, clearer gauge colours, shared hero plates
...
- **Issues inbox** — severity groups only (Critical / Warning / Info); drift / silent-fail / world-pressure / soft-hang still show as Issues with the right Fix actions
...
- **Help Center** — searchable in-app wiki
...
- **Soft-disable stays Disabled** — after rename to `.jar.disabled`, the Mods detail panel no longer snaps back to Enabled; no duplicate catalog row for template `mods.toml` junk
- **Remember me** — survives a server restart (sessions saved under `watchtower/dashboard-sessions.json`)

## [1.1.9] — 2026-07-29

> **Internal only** — interim build. Notes are under [1.2.0-beta.1](#120-beta1--2026-08-02).
