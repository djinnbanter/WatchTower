/**
 * Apply a named preview profile to dashboard-alpha fixtures.
 *
 * Usage:
 *   node scripts/apply-alpha-profile.mjs [profile]
 *   PREVIEW_PROFILE=fresh node scripts/apply-alpha-profile.mjs
 *
 * Profiles (see data/alpha-profiles.json):
 ...
 *   node scripts/apply-alpha-profile.mjs high-load
 *   npm run preview
 */
...
async function main() {
  console.log(`apply-alpha-profile: ${requestedProfile}`);
  ...
  console.log(`apply-alpha-profile: applied "${requestedProfile}" transforms`);
}
