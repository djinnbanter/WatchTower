...
import { AlertTriangle, Activity, Sparkles } from '@/ui/icons';
...
export function PatternsIncidents({ dash }: { dash: Record<string, unknown> }) {
  ...
  return (
    <PanelShell>
      ...
      {related.length ? (
        <FadeIn>
          <Section title="Related events" icon={Sparkles} hint="Lag spikes and nearby ops events.">
            <Stagger className="grid gap-2">
      ...
      <FadeIn>
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <Section title="Outlier minutes" icon={Sparkles}>
            {outliers.length ? (
