...
function ramSizingBadge(verdict: string): { label: string; tone: 'ok' | 'warn' | 'info' | 'neutral' } {
  switch (verdict) {
...
function RamSizingCard({
  ...
  const badge = ramSizingBadge(str(ram.verdict));
  ...
  return (
    <div className="relative rounded-[var(--radius-wt)] border border-wt-line bg-wt-bg1/90 p-5">
      ...
      <p className="text-sm text-wt-text-mid">{str(ram.advice, '—')}</p>
      <p className="mt-1 text-xs text-wt-text-low">Metrics below are for this Insights window.</p>
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-wt-text-low">
        {xmx != null ? (
          <span>
        ...
        {ram.headroom_gb != null ? (
          <span>
            Headroom <strong className="text-wt-text">{formatGb(num(ram.headroom_gb))}</strong>
          </span>
        ) : null}
        {suggestLabel ? (
          <span>
      ...
      {pasteNote ? <p className="mt-3 text-xs text-wt-text-mid">{pasteNote}</p> : null}
