...
function hourClock(hourUtc: number) {
          ...
          <div className="in-peak-card__title-wrap">
            ...
            <div>
              ...
              <p className="in-peak-card__sub">
                {busy ? 'Peak concurrent load windows (UTC)' : 'Lowest concurrent load windows (UTC)'}
              </p>
            </div>
          </div>
...
export function PatternsSchedule({ dash }: { dash: Record<string, unknown> }) {
  const how = asArray<Record<string, unknown>>(dash.hour_of_week);
  const busy = asArray<Record<string, unknown>>(get(dash, 'busy_quiet', 'busy_hours'));
  const quiet = asArray<Record<string, unknown>>(get(dash, 'busy_quiet', 'quiet_hours'));
    ...
    <PanelShell>
      <FadeIn>
        <Stagger className="in-schedule-peaks" delayMs={55}>
          <PeakHoursCard variant="busy" hours={busy} />
          <PeakHoursCard variant="quiet" hours={quiet} />
        </Stagger>
      </FadeIn>

      <FadeIn>
        <Section title="MSPT heatmap" icon={Activity} hint="Day × hour (UTC). Cool = smooth, red = tick lag.">
      ...
      <FadeIn>
        <Section title="TPS heatmap" icon={Activity} hint="Day × hour (UTC). Green = full rate, red = dropped ticks.">
      ...
      <FadeIn>
        <Section title="Players heatmap" icon={Users} hint="Day × hour (UTC). Blue intensity = busier (not a health grade).">
      ...
      <FadeIn>
        <Section
          title="Hourly averages (UTC)"
          icon={Clock}
          hint="Hour-of-day averages across the window. Hover a bar for that UTC hour."
        >
