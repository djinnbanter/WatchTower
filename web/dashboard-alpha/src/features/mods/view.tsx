...
import { ErrorState, HeroTabNav, MetricReadout, StatusPill } from '@/ui/patterns';
...
function heroGlowProps(tone: 'ok' | 'warn' | 'danger' | 'info') {
  if (tone === 'danger') {
    return {
      glowColor: '0 84 60',
      glowIntensity: 0.5,
  ...
  if (tone === 'info') {
    return {
      glowColor: '210 80 55',
      glowIntensity: 0.45,
      colors: ['#60a5fa', '#22d3ee', '#a78bfa'] as string[],
      fillOpacity: 0.15,
      backgroundColor: 'var(--wt-bg1)',
    };
  }
...
export function PageView({ route }: { route: RouteState }) {
  ...
  const securityIds = badgeMaps.securityFlags
    ...
    .filter(Boolean);
  const connectorIds = badgeMaps.connectorWarnings
  ...
  const showStatusStrip =
    securityIds.length > 0 || connectorIds.length > 0 || view === 'overview' || view === 'modrinth';
          ...
          <div className="md-hero__body">
            <div className="md-hero__main">
              ...
              <HeroTabNav
                ...
                onChange={(id) => navigate({ tab: 'mods', view: id, mod: null })}
              />
                  ...
                  {connectorIds.length ? (
                    ...
                    >
                      ...
                      <span className="md-status__text">
                        Fabric-side analogues: {connectorIds.join(', ')}
                      </span>
                    </button>
                  ) : null}
                  {view === 'overview' || view === 'modrinth' || view === 'updates' ? (
