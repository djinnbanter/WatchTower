...
import { FadeIn, GlareIcon, PageEnter, Stagger, useCountUp } from '@/ui/motion';
...
export function PageView({ route: _route }: { route: RouteState }) {
  ...
  const joinCount = events.filter((e) => e.type === 'player_join').length;
          ...
          <div className="ac-hero__body">
            <div className="ac-hero__main">
              ...
              <p className="ac-hero__hint">
                Commands, joins, lag, backups, and jobs from the latest ops scan. Filter or search to
                find a moment.
              </p>
            </div>
            ...
            <div className="ac-kpis" aria-label="Activity vitals">
              ...
              <Kpi
                label="Joins"
                value={joinCount}
                hint="Player join events"
                tone={joinCount ? 'ok' : 'default'}
              />
              <Kpi
                label="Latest"
