...
import { Archive, Check, RefreshCw, Search, Shield, SlidersHorizontal, Sparkles } from '@/ui/icons';
import { PageEnter, SpotlightCard } from '@/ui/motion';
import { Button } from '@/ui/patterns';
import { asArray, asRecord, bool, num, str, totpQrSrc } from '@/lib/utils';

const STEPS = [
  { id: 'welcome', title: 'Welcome to WatchTower', icon: Sparkles },
  ...
  return (
    <PageEnter className="mx-auto flex min-h-dvh max-w-2xl flex-col justify-center px-4 py-10">
      <SpotlightCard className="overflow-hidden">
        <div className="bg-gradient-to-br from-wt-accent-soft via-transparent to-transparent p-8">
          <div className="mx-auto grid h-14 w-14 place-items-center rounded-3xl bg-wt-accent/15 text-wt-accent">
            <Icon size={26} />
          </div>
          <div className="mx-auto mt-4 h-1.5 w-full max-w-xs overflow-hidden rounded-full bg-wt-bg2">
            <div
              className="h-full rounded-full bg-wt-accent transition-[width] duration-300"
              style={{ width: `${((step + 1) / STEPS.length) * 100}%` }}
            />
          </div>
          <p className="mt-4 text-center text-xs font-semibold uppercase tracking-[0.16em] text-wt-text-low">
            Step {step + 1} of {STEPS.length}
          </p>
          <h2 className="mt-2 text-center text-2xl font-bold tracking-tight">{current.title}</h2>
...
function OptionsStep() {
  ...
  return (
    <div className="space-y-3 text-sm text-wt-text-mid">
      ...
      <label className="flex items-start gap-3 rounded-xl border border-wt-line bg-wt-bg2/50 p-3">
        <input
          type="checkbox"
          className="mt-1"
          checked={lookup}
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
      {tiles.map((t) => (
        <div
          key={t.key}
          className={`rounded-xl border border-wt-line bg-wt-bg2/50 px-3 py-2 text-center ${
...
function DiscoveryStep({ onCompleteChange }: { onCompleteChange: (ok: boolean) => void }) {
  ...
  return (
    <div className="space-y-4 text-sm text-wt-text-mid">
      ...
      {(running || starting) && (
        <div className="space-y-2 rounded-xl border border-wt-line bg-wt-bg2/40 p-3">
...
function BackupsStep({
  ...
  return (
    <div className="space-y-4 text-sm text-wt-text-mid">
      ...
      <div className="rounded-xl border border-wt-line bg-wt-bg2/40 p-4">
        <LocalFolderSetup
  ...
  return (
    <div className="space-y-3 text-sm text-wt-text-mid">
      ...
      <ul className="space-y-2">
        <li className="flex items-start gap-2 rounded-xl border border-wt-line bg-wt-bg2/40 px-3 py-2">
      ...
      {!totpEnabled && totpStep === 'idle' ? (
        <div className="space-y-2 rounded-xl border border-wt-line bg-wt-bg2/40 p-4">
          <p className="text-xs text-wt-text-low">
      ...
      {totpStep === 'qr' ? (
        <div className="space-y-3 rounded-xl border border-wt-line bg-wt-bg2/40 p-4">
          ...
          <label className="block text-sm">
            ...
            <input
              className="mt-1.5 w-full rounded-xl border border-wt-line bg-wt-bg1 px-3 py-2 font-mono text-sm outline-none focus:border-wt-accent"
              value={totpCode}
