...
import { ChevronRight, ExternalLink, FileText, HardDriveDownload, RefreshCw, Send, Sparkles, Zap } from '@/ui/icons';
import { FadeIn, PageEnter, ShimmerText } from '@/ui/motion';
import { Button, EmptyState, ErrorState, HeroTabNav, StatusPill } from '@/ui/patterns';
...
/** React Bits BorderGlow props keyed to verdict grade (Overview mission twin). */
...
export function PageView({ route }: { route: RouteState }) {
  ...
  if (!enabled) {
    return (
      <PageEnter className="sp-stack">
        <BorderGlow className="sp-disabled" borderRadius={14} animated>
          <Sparkles size={24} />
          <div>
  ...
  return (
    <PageEnter className="sp-stack">
      <BorderGlow
        ...
        {...missionBorderGlowProps(glowTone)}
      >
        <div className="sp-hero__surface">
          <div className="sp-hero__head">
            <div className="sp-hero__identity">
              <span className="sp-hero__icon" aria-hidden><Sparkles size={18} /></span>
            ...
            <div className="sp-hero__actions">
              {text(loadedVerdict.grade) ? (
                <StatusPill tone={gradePillTone(text(loadedVerdict.grade))}>
                  {glowTone === 'danger'
                    ? <ShimmerText>{text(loadedVerdict.grade)}</ShimmerText>
                    : text(loadedVerdict.grade)}
                </StatusPill>
              ) : null}
