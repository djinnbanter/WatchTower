...
const SparklesIcon = Sparkles as SectionIcon;
      ...
      <div className="sp-brief-grid">
        ...
        {topSource ? (
          <div className={`relative rounded-[var(--radius-wt)] border border-wt-line bg-wt-bg1/90 shadow-[var(--wt-shadow)] overflow-hidden sp-brief-card sp-brief-card--source${topSource.ownPct >= 25 ? ' is-hot' : ''}`}>
            <div className="sp-brief-card__head">
              <GlareIcon icon={Sparkles} tone={topSource.ownPct >= 25 ? 'warn' : 'accent'} />
              <div className="sp-brief-card__titles">
                  ...
                  <div className="sp-next-step__inner">
                    ...
                    <div className="sp-next-step__body">
                      <div className="sp-next-step__meta">
                        <GlareIcon icon={Sparkles} tone={iconTone} />
      ...
      <Section title="Busy chunks" hint="Block ranges are approximate (chunk × 16). Open a chunk to see every entity type counted there.">
        <div className="sp-hotspot-grid">
          {hotspots.map((row, index) => {
            ...
            return (
              <div className="relative rounded-[var(--radius-wt)] border border-wt-line bg-wt-bg1/90 shadow-[var(--wt-shadow)] overflow-hidden sp-hotspot-card" key={`${text(row.dimension)}:${numeric(row.chunk_x)}:${numeric(row.chunk_z)}:${index}`}>
                <StatusPill tone="warn">{worldDimensionLabel(text(row.dimension))}</StatusPill>
                <h3>Chunk {numeric(row.chunk_x)}, {numeric(row.chunk_z)}</h3>
                <p>
                ...
                <Fact
                  ...
                  value={row.nearest_player_chunk_distance != null
                ...
                <div className="sp-hotspot-card__actions">
                  <Button kind="default" onClick={() => setSelectedHotspot(row)}>
                    Inspect chunk
                  </Button>
                </div>
              </div>
          ...
          {!hotspots.length ? <EmptyState title="No busy chunks listed">This capture didn’t include chunk entity maps.</EmptyState> : null}
        </div>
      </Section>
            ...
            {topType ? (
              <div className="sp-chunk-modal__lead">
                <div className="sp-chunk-modal__lead-head">
                  <GlareIcon icon={Sparkles} tone="warn" />
                  <div>
      ...
      {otherPath && otherQ.data ? (
        <>
          <div className={`relative rounded-[var(--radius-wt)] border border-wt-line bg-wt-bg1/90 shadow-[var(--wt-shadow)] overflow-hidden sp-alert-card${!compatible || compatibility.length ? ' is-warning' : ' is-ok'}`}>
            {!compatible || compatibility.length ? <Info size={14} /> : <Sparkles size={14} />}
            <p>
              {!compatible
