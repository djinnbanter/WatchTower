...
import { Button, ErrorState, HeroTabNav, MetricReadout, StatusPill } from '@/ui/patterns';
...
function heroGlowProps(critical: number, warning: number) {
  if (critical > 0) {
    return {
      glowColor: '0 84 60',
      glowIntensity: 0.5,
...
export function PageView({ route }: { route: RouteState }) {
  ...
  const { dismissInbox, undismissInbox } = useSessionStore();
    ...
    <PageEnter className="is-stack">
      <FadeIn>
        <BorderGlow
          ...
          coneSpread={18}
          animated
          {...heroGlowProps(heroCritical, heroWarning)}
        >
            ...
            <div className="is-hero__main">
              ...
              <div className="is-hero__search-row">
                ...
                {resolvedView === 'active' && criticalCount + warningCount + infoCount > 0 ? (
                  <Button disabled={busy} onClick={() => markAll(markAllKeys)}>
                    Mark all reviewed
                  </Button>
                ) : null}
              </div>
