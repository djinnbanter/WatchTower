...
import { PortCallout } from '@/components/how/port-callout';
import { howNightById } from '@/content/how-night';
import { HOW } from '@/content/how-it-works';

const meta = howNightById('desk');

export function HowDeskEntry() {
  return (
    <ShiftEntry {...meta}>
      <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] lg:gap-12">
        <div className="min-w-0">
          <h2 className="wt-entry text-[color:var(--wt-text)]">{HOW.desk.title}</h2>
          <p className="mt-4 max-w-[48ch] text-[1.0625rem] leading-relaxed text-[color:var(--wt-text-mid)]">
            {HOW.desk.capability}
          </p>
          <TourBrings items={HOW.desk.brings} />
          <MarginNote className="mt-5">{HOW.desk.note}</MarginNote>
        </div>
        <div className="min-w-0">
          <PortCallout className="w-full" />
        </div>
      </div>
    </ShiftEntry>
  );
