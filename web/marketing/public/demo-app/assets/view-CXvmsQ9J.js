...
**Activity** is a timeline of commands, joins, lag, and jobs from the latest ops scan — useful for “what happened around then?”
...
## Activity vs Insights vs Issues

| Tab | Job |
|-----|-----|
| **Activity** | Event timeline / incident story |
...
- KPI summary for the recent window
- Typed timeline (filter chips by event kind)
- Incident story deep links into related tabs

Events come from **Scanning** (and gap backfill). Freshness: [[Sources]].

---

## What to do next

1. Filter to the event types you care about
2. Follow deep links into [[Crashes]], [[Mods]], or [[Backups]]
3. If the timeline is thin, wait for Scanning or click Refresh
...
Kill-switches: \`MOD_JAR_DRIFT_ENABLED\`, \`CLIENT_ON_SERVER_ISSUES_ENABLED\`, \`SILENT_FAIL_DETECT_ENABLED\`, \`WORLD_PRESSURE_ENABLED\`, \`JOIN_CLINIC_ENABLED\` in \`watchtower.conf\` — see [[Configuration]].
...
`}}};function le(s){return String(s??"").split("#")[0].trim().replace(/\s+/g,"-")}function R(s){const n=s.trim();return n.startsWith("|")&&n.includes("|",1)}function T(s){const n=s.trim();return n.includes("-")?/^\|?[\s:|-]+\|?$/.test(n):!1}function P(s){let n=s.trim();return n.startsWith("|")&&(n=n.slice(1)),n.endsWith("|")&&(n=n.slice(0,-1)),n.split("|").map(t=>t.trim())}function ce({slug:s,label:n}){const t=le(s);return e.jsx("a",{className:"wiki-link",href:V("docs",{wiki:t}),onClick:r=>{r.preventDefault(),w({tab:"docs",wiki:t})},children:n})}function u(s){if(!s)return null;const n=[],t=/\[\[([^\]|]+)(?:\|([^\]]+))?\]\]|`([^`]+)`|\*\*([^*]+)\*\*|\*([^*]+)\*|\[([^\]]+)\]\(([^)]+)\)/g;let r=0,i,a=0;for(;(i=t.exec(s))!==null;){if(i.index>r&&n.push(s.slice(r,i.index)),i[1]!=null){const l=i[1].trim(),o=(i[2]||i[1].split("#")[0]).trim();n.push(e.jsx(ce,{slug:l,label:o},a++))}else if(i[3]!=null)n.push(e.jsx("code",{className:"wiki-inline-code",children:i[3]},a++));else if(i[4]!=null)n.push(e.jsx("strong",{children:i[4]},a++));else if(i[5]!=null)n.push(e.jsx("em",{children:i[5]},a++));else if(i[6]!=null){const l=i[7],o=/^https?:\/\//.test(l);n.push(e.jsx("a",{className:"wiki-ext-link",href:l,target:o?"_blank":void 0,rel:o?"noopener noreferrer":void 0,children:i[6]},a++))}r=t.lastIndex}return r<s.length&&n.push(s.slice(r)),n.length===1?n[0]:n}function de(s){const n=[];let t=0,r=0;for(;t<s.length;){const i=s[t];if(!i.trim()){t++;continue}if(i.startsWith("```")){const o=i.slice(3).trim(),c=[];for(t++;t<s.length&&!s[t].startsWith("```");)c.push(s[t]),t++;t++,n.push(e.jsx("pre",{className:`wiki-code-block${o?` lang-${o}`:""}`,children:e.jsx("code",{children:c.join(`
`)})},r++));continue}const a=i.match(/^(#{1,6})\s+(.+)$/);if(a){const o=a[1].length,c=a[2].trim(),d=c.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");n.push(p.createElement(`h${o}`,{key:r++,id:d,className:`wiki-h${o}`},u(c))),t++;continue}if(/^[-*_]{3,}\s*$/.test(i)&&!i.includes("|")){n.push(e.jsx("hr",{className:"wiki-hr"},r++)),t++;continue}if(R(i)&&t+1<s.length&&T(s[t+1])){const o=P(i);t+=2;const c=[];for(;t<s.length&&R(s[t])&&!T(s[t]);)c.push(P(s[t])),t++;n.push(e.jsx("div",{className:"wiki-table-wrap",children:e.jsxs("table",{className:"wiki-table",children:[e.jsx("thead",{children:e.jsx("tr",{children:o.map((d,h)=>e.jsx("th",{children:u(d)},h))})}),e.jsx("tbody",{children:c.map((d,h)=>e.jsx("tr",{children:d.map((S,_)=>e.jsx("td",{children:u(S)},_))},h))})]})},r++));continue}if(/^>\s?/.test(i)){const o=[];for(;t<s.length&&/^>\s?/.test(s[t]);)o.push(s[t].replace(/^>\s?/,"")),t++;n.push(e.jsx("blockquote",{className:"wiki-callout wiki-blockquote",children:u(o.join(" "))},r++));continue}if(/^[-*+]\s+\[[ xX]\]\s/.test(i)){const o=[];for(;t<s.length&&/^[-*+]\s+\[[ xX]\]\s/.test(s[t]);){const c=/\[[xX]\]/.test(s[t]),d=s[t].replace(/^[-*+]\s+\[[ xX]\]\s/,"");o.push(e.jsxs("li",{className:c?"wiki-checklist__item wiki-checklist__item--done":"wiki-checklist__item",children:[e.jsx("input",{type:"checkbox",disabled:!0,checked:c,"aria-hidden":!0}),e.jsx("span",{children:u(d)})]},o.length)),t++}n.push(e.jsx("ul",{className:"wiki-checklist",children:o},r++));continue}if(/^[-*+]\s/.test(i)){const o=[];for(;t<s.length&&/^[-*+]\s/.test(s[t])&&!/^[-*+]\s+\[[ xX]\]\s/.test(s[t]);)o.push(e.jsx("li",{children:u(s[t].replace(/^[-*+]\s/,""))},o.length)),t++;n.push(e.jsx("ul",{className:"wiki-ul",children:o},r++));continue}if(/^\d+\.\s/.test(i)){const o=[];for(;t<s.length&&/^\d+\.\s/.test(s[t]);)o.push(e.jsx("li",{children:u(s[t].replace(/^\d+\.\s/,""))},o.length)),t++;n.push(e.jsx("ol",{className:"wiki-ol wiki-ol--steps",children:o},r++));continue}const l=[];for(;t<s.length&&s[t].trim()&&!/^#{1,6}\s/.test(s[t])&&!/^[-*+]\s/.test(s[t])&&!/^\d+\.\s/.test(s[t])&&!s[t].startsWith("```")&&!(/^[-*_]{3,}\s*$/.test(s[t])&&!s[t].includes("|"))&&!/^>\s?/.test(s[t])&&!(R(s[t])&&t+1<s.length&&T(s[t+1]));)l.push(s[t]),t++;l.length?n.push(e.jsx("p",{className:"wiki-p",children:u(l.join(" "))},r++)):t++}return n}function he(s){if(!s)return{lead:null,body:""};const n=s.replace(/\r\n/g,`
`).replace(/\r/g,`
`),t=n.split(`
