...
const previewSuppressionState = new Set();
const previewSuppressionConfig = ['CLIENT_ON_SERVER'];

function previewSuppressions() {
  const state = [...previewSuppressionState];
  return {
    conf_ids: [...previewSuppressionConfig],
    state,
    merged: [
      ...previewSuppressionConfig.map((id) => ({ id, source: 'conf' })),
      ...state
        .filter((id) => !previewSuppressionConfig.includes(id))
        .map((id) => ({ id, source: 'state' })),
    ],
  };
}
...
export async function suppressIssue(payload) {
  if (isFixturePreview()) {
    if (payload?.issue_id) previewSuppressionState.add(payload.issue_id);
    return { ok: true, suppressions: previewSuppressions() };
  }
...
export async function unsuppressIssue(payload) {
  if (isFixturePreview()) {
    const removed = previewSuppressionState.delete(payload?.issue_id);
    return { ok: true, removed, suppressions: previewSuppressions() };
  }
...
export async function incidents() {
  if (isFixturePreview()) {
    return loadFixtureJson('data/incidents-index.json');
  }
...
export async function incidentGet(id) {
  if (isFixturePreview()) {
    return loadFixtureJson(`data/incidents/${encodeURIComponent(id)}.json`);
  }
