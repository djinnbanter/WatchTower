...
import {
  ...
  modrinthScan, discovery, incidents,
...
const PATHS = {
  ...
  activeProfile:       'data/active-profile.json',
  modrinthStatus:      'data/modrinth-status.json',
...
export class FixtureSource {
  constructor() {
    ...
    this._simState = null;
    this._profile = { name: 'normal', behavior: {} };
  }

  // ── Boot / initial load ────────────────────────────────────────────────────

  async boot() {
    this._profile = await loadJson(PATHS.activeProfile).catch(() => null)
      ?? { name: 'normal', behavior: {} };
    const bootDelay = Math.max(0, Math.min(5000, Number(this._profile?.behavior?.boot_delay_ms) || 0));
    if (bootDelay) {
    ...
    const [
      ...
      loadJson(PATHS.modrinthStatus).catch(() => null),
      loadJson(PATHS.incidentsIndex).catch(() => null),
    ]);
    ...
    live.value = {
      ...
      latest,
      error: this._profile?.behavior?.live_error ?? null,
      at: Date.now(),
    };
    ...
    dataSources.value = {
      liveAt: latest?.ts ?? latest?.at ?? latest?.polled_at ?? envelope?.latest?.at ?? null,
    ...
    performance.value = {
      ...
      at: Date.now(),
      error: this._profile?.behavior?.performance_error ?? null,
    };

    if (modrinthStatusData) {
      modrinthScan.value = {
        ...modrinthScan.value,
        status: { ...modrinthScan.value.status, ...modrinthStatusData },
      };
    }
    const incidentRows = incidentsIndexData?.incidents ?? [];
    incidents.value = {
      list: incidentRows,
      byId: Object.fromEntries(incidentRows.map((row) => [row.id, row])),
    };

    // Reports
    await this._loadReports(index);
    if (this._profile?.behavior?.report_running) {
      reports.value = {
        ...reports.value,
        run: {
          running: true,
          startedAt: Date.now() - 2500,
          message: 'Support compose in progress (preview)',
          success: null,
          stage: 'collect',
          stageLabel: 'Collecting logs, crashes, mods, host metrics',
          stageDetail: 'Scanning server logs…',
        },
      };
    }
    ...
    // Seed settings from preview fixture when store is empty
    if (!settings.value?.data || Object.keys(settings.value.data).length === 0) {
      const previewSettings = await loadJson('data/preview-settings.json').catch(() => null);
      if (previewSettings) {
        settings.value = { data: previewSettings, error: null, at: Date.now() };
      }
    }
  ...
  async fetchModrinthStatus() {
    const fixture = await loadJson(PATHS.modrinthStatus).catch(() => null);
    const data = {
      ...(fixture ?? {}),
      enabled: settings.value?.data?.modrinth_lookup !== false,
      running: modrinthScan.value?.status?.running ?? fixture?.running ?? false,
      stage: modrinthScan.value?.status?.stage ?? fixture?.stage ?? null,
      stage_label: modrinthScan.value?.status?.stage_label ?? fixture?.stage_label ?? null,
      stage_detail: modrinthScan.value?.status?.stage_detail ?? fixture?.stage_detail ?? null,
      progress: modrinthScan.value?.status?.progress ?? fixture?.progress ?? { done: 0, total: 0 },
      batch: modrinthScan.value?.status?.batch ?? fixture?.batch ?? { index: 0, count: 0, size: 0 },
      eta_seconds: modrinthScan.value?.status?.eta_seconds ?? fixture?.eta_seconds ?? null,
      last_run: modrinthScan.value?.status?.last_run ?? fixture?.last_run ?? null,
      history: modrinthScan.value?.status?.history ?? fixture?.history ?? [],
  ...
  async runModrinthScan() {
    ...
    modrinthScan.value = {
      ...
      status: {
        ...
        history: modrinthScan.value.status?.history ?? [],
    ...
    const tick = () => {
      ...
      const history = [
        ...(modrinthScan.value.status?.history ?? []),
        {
          started_at: started,
  ...
  async fetchSettings() {
    if (!settings.value?.data || Object.keys(settings.value.data).length === 0) {
      const preview = await loadJson('data/preview-settings.json').catch(() => null);
      if (preview) {
        settings.value = { ...settings.value, data: preview, error: null, at: Date.now() };
      }
    }
  ...
  async pinIncident() {
    const now = new Date();
    const id = `manual-${now.toISOString().replace(/[:.]/g, '-')}`;
    ...
    const incident = {
      ...
      source: 'manual',
      trigger: 'manual',
      severity: (latest.mspt ?? 0) >= 50 || (latest.tps ?? 20) < 18 ? 'critical' : 'info',
      tps: latest.tps ?? null,
      mspt: latest.mspt ?? null,
      players_online: latest.players_online ?? null,
      narrative: 'Lag moment pinned during this preview session.',
    };
    incidents.value = {
      list: [incident, ...(incidents.value.list ?? [])],
      byId: { ...(incidents.value.byId ?? {}), [id]: incident },
    };
    return { id, incident, preview: true };
  }

  async fetchIncidents() {
    return { incidents: incidents.value.list ?? [] };
  }

  async fetchIncident(id) {
    const cached = incidents.value.byId?.[id];
    if (cached) return cached;
    const data = await loadJson(`data/incidents/${id}.json`).catch(() => null);
    if (data) {
      incidents.value = {
        list: incidents.value.list?.some((row) => row.id === id)
          ? incidents.value.list
          : [data, ...(incidents.value.list ?? [])],
        byId: { ...(incidents.value.byId ?? {}), [id]: data },
      };
    }
    return data;
  }
  ...
  startSimulator(intervalMs = 3000) {
    if (this._profile?.behavior?.simulator === false) return;
