#!/usr/bin/env node
/**
 * Static smoke checks for React dashboard.
 */
...
check(exists('data/preview-profiles.json'), 'preview-profiles');
...
check(exists('src/ui/patterns/specular-cta.tsx'), 'SpecularCtaButton (CSS-only)');
...
check(!/Commons Clause/i.test(read('src/ui/patterns/specular-cta.tsx')), 'SpecularCta no Commons Clause');
check(/wt-specular-cta/.test(read('src/ui/patterns/specular-cta.tsx')), 'SpecularCta uses CSS classes');
check(!pkg.dependencies?.ogl, 'no ogl dependency');
check(!exists('src/components/specular-button/SpecularButton.tsx'), 'SpecularButton removed');
check(!pkg.dependencies?.gsap, 'no gsap dependency');
check(!/\b(SpotlightCard|ShimmerText|Magnet|ClickSpark)\b/.test(read('src/ui/motion/index.tsx')), 'heavy motion primitives removed');
