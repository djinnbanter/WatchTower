...
const ChartCore = memo(function ChartCore({
  ...
  // Stable synthetic dates so categorical bars can share the time-scale ChartContext API.
  const categoryDates = useMemo(() => {
    const now = Date.now();
    const dayMs = 24 * 60 * 60 * 1000;
    return data.map((_, i) => new Date(now - (data.length - 1 - i) * dayMs));
  }, [data]);
  ...
  const xAccessorDate = useCallback(
    (d: Record<string, unknown>): Date => {
      ...
      return categoryDates[idx] ?? categoryDates[0] ?? new Date();
    },
    [xDataKey, data, categoryDates]
  );
  ...
  const yScales = useMemo(() => {
    // Stacked bars share one value scale (domain = per-row series sum). Using
    // per-series max here collapses the axis to a single segment height and
    // pushes upper stack segments off-chart.
    if (isHorizontal || stacked) {
  ...
  // Callable fake time scale — spreading scaleBand yields a non-callable object
  // and crashes XAxis / tooltip paths with "xScale is not a function".
  const fakeTimeScale = useMemo(() => {
    const start = categoryDates[0] ?? new Date();
    const end = categoryDates[categoryDates.length - 1] ?? start;
    const startMs = start.getTime();
    const endMs = end.getTime();
    const span = Math.max(1, endMs - startMs);
    const axisRange: [number, number] = isHorizontal
      ? [0, innerHeight]
      : [0, innerWidth];

    const scaleFn = ((value: Date | number) => {
      const ms = value instanceof Date ? value.getTime() : Number(value);
      const idx = categoryDates.findIndex((d) => d.getTime() === ms);
      if (idx >= 0) {
        const row = data[idx];
        if (row) {
          const band = categoryScale(categoryAccessor(row));
          if (band != null) {
            return band + bandWidth / 2;
          }
        }
      }
      return ((ms - startMs) / span) * axisRange[1];
    }) as ((value: Date | number) => number) & {
      domain: () => Date[];
      range: () => [number, number];
    ...
    scaleFn.domain = () => [start, end];
    scaleFn.range = () => axisRange;
    scaleFn.invert = (x: number) => new Date(startMs + (x / Math.max(1, axisRange[1])) * span);
    scaleFn.copy = () => scaleFn;
    scaleFn.ticks = (count = 5) => {
      if (categoryDates.length <= count) {
        return [...categoryDates];
      }
      const step = Math.max(1, Math.floor((categoryDates.length - 1) / Math.max(1, count - 1)));
      const ticks: Date[] = [];
      for (let i = 0; i < categoryDates.length; i += step) {
        const d = categoryDates[i];
        if (d) ticks.push(d);
      }
      const last = categoryDates[categoryDates.length - 1];
