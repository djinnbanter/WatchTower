...
export function useChartInteraction({
  ...
  /** Stable sample time for re-anchor across sliding xDomain (not drifting pixel X). */
  ...
  const resolveTooltipFromTime = useCallback(
    (timeMs: number): TooltipData | null => {
      if (data.length === 0) return null;
      const index = bisectDate(data, new Date(timeMs), 1);
      const d0 = data[index - 1];
      const d1 = data[index];
      if (!d0) return null;
      ...
      if (d1) {
        const d0Time = xAccessor(d0).getTime();
        const d1Time = xAccessor(d1).getTime();
        if (Math.abs(timeMs - d1Time) < Math.abs(timeMs - d0Time)) {
          d = d1;
          finalIndex = index;
        }
      }
  ...
  const tooltipDedupeKey = useCallback(
    (tooltip: TooltipData): string => {
      const t = xAccessor(tooltip.point).getTime();
      return Number.isFinite(t) ? `t:${t}` : String(tooltip.index);
    },
    [xAccessor]
  );
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent<SVGGElement>) => {
      ...
      if (tooltip) {
        lastHoveredTimeRef.current = xAccessor(tooltip.point).getTime();
        scheduleTooltip(tooltip, tooltipDedupeKey(tooltip));
      }
    },
    [
      getChartX,
  ...
  // Re-anchor tooltip/crosshair by sample time when x-scale or visible data changes
  // (live slide / brush). Never clear on a transient miss — that flickers SeriesHoverDim.
  useEffect(() => {
    if (!canInteract || lastHoveredTimeRef.current === null) {
      return;
    }
    const tooltip = resolveTooltipFromTime(lastHoveredTimeRef.current);
    if (tooltip) {
      scheduleTooltip(tooltip, tooltipDedupeKey(tooltip));
    }
  }, [
    canInteract,
    resolveTooltipFromTime,
    scheduleTooltip,
    tooltipDedupeKey,
  ]);
