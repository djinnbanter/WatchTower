...
export function TimeSeriesChartInner(props: TimeSeriesChartInnerProps) {
  ...
  const size = lastGoodSizeRef.current;
  if (width < 10 || height < 10) {
    if (!size) return null;
    return <TimeSeriesChartCore {...props} width={size.width} height={size.height} />;
  }
...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
  ...
  const skeletonData = useMemo(() => {
    const dataKeys = lines.map((line) => line.dataKey).filter(Boolean);
    const primaryKey = dataKeys[0] ?? "value";
    if (data.length === 0) {
      return generateChartSkeletonData({ dataKey: primaryKey });
    }
    return generateChartSkeletonFromTarget(data, dataKeys);
  }, [data, lines]);
  ...
  // until the window span changes (preset / custom range length), with slow recover
  // so a spike does not permanently crush the line.
  ...
  const liveYDomainRecoverRef = useRef<{ belowSinceMs: number | null }>({
    belowSinceMs: null,
  });
  ...
  const yDomainTargetByAxis = useMemo(() => {
    ...
    if (!liveYDomainLock) {
      ...
      liveYDomainRecoverRef.current.belowSinceMs = null;
      return merged;
    }
    ...
    const locked = expandOnlyYDomainsWithRecover(
      prev.domains,
      merged,
      liveYDomainRecoverRef.current,
      Date.now(),
    );
