...
import { seriesAreaPathFromPoints } from "./series-path-utils";
...
export function Area({
  ...
  // Discrete xDomain changes (brush release / presets) morph the path.
  const useDataTransitionPath = animate && chartPhase === "ready";
  const { pathD: animatedPathD, points: animatedPoints } =
  ...
  const animatedAreaPathD = useMemo(() => {
    if (!useDataTransitionPath || animatedPoints.length === 0) {
      return "";
    }
    const baseline =
      typeof (yScale as { range?: () => number[] }).range === "function"
        ? ((yScale as { range: () => number[] }).range()[0] ?? innerHeight)
        : innerHeight;
    return seriesAreaPathFromPoints(animatedPoints, curve, baseline);
  }, [animatedPoints, curve, innerHeight, useDataTransitionPath, yScale]);
  const {
    handleLoadingPulseComplete,
    pulseMode,
    pulseEpoch,
  ...
  const seriesLayers = (
    <>
      {showSeriesContent && showAreaFill ? (
        useDataTransitionPath && animatedAreaPathD ? (
          <path d={animatedAreaPathD} fill={areaFill} stroke="none" />
        ) : (
          <AreaClosed
            curve={curve}
            data={renderData}
      ...
      {shouldMeasurePath ? (
        <>
          {useDataTransitionPath && animatedPathD ? (
            <path
              d={animatedPathD}
              fill="none"
              ref={pathRef}
