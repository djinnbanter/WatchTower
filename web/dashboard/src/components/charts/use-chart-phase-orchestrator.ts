...
/** Escape hatch if pulse / conceal callbacks never fire (huge series, zero width). */
const PHASE_WATCHDOG_MS = 3500;
...
export function useChartPhaseOrchestrator({
  ...
  // If exit/conceal/grid callbacks stall, force the next resting phase.
  useEffect(() => {
    if (
      chartPhase !== "exiting" &&
      chartPhase !== "exitingReady" &&
      chartPhase !== "gridTweenLoading" &&
      chartPhase !== "gridTweenReady"
    ) {
      return;
    }

    const stuckPhase = chartPhase;
    const timer = window.setTimeout(() => {
      if (phaseRef.current !== stuckPhase) {
        return;
      }
      if (stuckPhase === "exiting" || stuckPhase === "gridTweenReady") {
        setChartPhase("revealing");
        return;
      }
      setChartPhase("loading");
