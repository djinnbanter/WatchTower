...
const SENTENCES: Record<string, (r: AuditRow) => string> = {
  ...
  account_capabilities_changed: (r) =>
    `${r.actor} changed capabilities for ${r.target ?? 'an account'}`.trim(),
  mod_disabled: (r) => `${r.actor} disabled jar ${r.target ?? ''}`.trim(),
  mod_enabled: (r) => `${r.actor} enabled jar ${r.target ?? ''}`.trim(),
  mod_swap: (r) => `${r.actor} applied a mod jar update${r.target ? ` (${r.target})` : ''}`.trim(),
  mod_batch: (r) => `${r.actor} applied a batch of mod updates${r.target ? ` (${r.target})` : ''}`.trim(),
  mod_install: (r) => `${r.actor} installed a mod jar${r.target ? ` (${r.target})` : ''}`.trim(),
  mod_quarantine: (r) => `${r.actor} quarantined jar ${r.target ?? ''}`.trim(),
  mod_swap_undo: (r) => `${r.actor} undid a mod jar change${r.target ? ` (${r.target})` : ''}`.trim(),
