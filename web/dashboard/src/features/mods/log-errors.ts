...
export function mergeLogErrorRows({
  ...
  hasReport: boolean;
  /** Mod ids whose Issues `mod:{id}` row was reviewed — omit from active / keep for reviewed. */
  ackedModIds?: Iterable<string> | null;
  /** `active` hides reviewed; `reviewed` only shows reviewed (with evidence still present). */
  band?: 'active' | 'reviewed';
}): LogErrorRow[] {
  const acked = new Set(
  ...
  for (const row of byId.values()) {
    const isAcked = acked.has(String(row.mod_id).toLowerCase());
    if (band === 'active' && isAcked) continue;
    if (band === 'reviewed' && !isAcked) continue;
