...
export function humanFailureLabel(
  ...
  external_kill_subtype?: string | null,
): string {
  ...
  const ekSubtype = String(external_kill_subtype || '').toLowerCase().trim();
  ...
  if (kind === 'external_kill') {
    if (ekSubtype === 'oom') return 'Killed by the OS out-of-memory killer';
    return 'Force-killed from outside the server';
  }
  if (kind === 'mod_runtime') {
...
export function buildFixPlan(
  ...
  if (!headline || headline.length > 140) {
    ...
    } else if (failureKind === 'external_kill') {
      const details = (s.details && typeof s.details === 'object' ? s.details : {}) as Record<string, unknown>;
      const ekSubtype = String(details.external_kill_subtype || s.subtype || '').toLowerCase();
      if (ekSubtype === 'oom') {
        headline =
          'Raise the container / host memory limit or lower -Xmx — check Insights → Configs for RAM sizing';
      } else {
        headline =
          'Raise the panel stop / watchdog timeout so a slow save is not force-killed';
      }
