...
export function fromLedgerRow(
  ...
  if (id === 'SOFT_HANG' || id.includes('SOFT_HANG')) {
    ...
    primaryAction = { label: 'Open Mods Changes', tab: 'mods', params: { view: 'changes' } };
  } else if (id.startsWith('SILENT_FAIL') || id.includes('SILENT_FAIL')) {
    primaryAction = { label: 'Open Logs', tab: 'logs' };
  } else if (id.startsWith('WORLD_PRESSURE') || id.includes('WORLD_PRESSURE')) {
    primaryAction = { label: 'Open World pressure', tab: 'insights', params: { view: 'world', panel: null } };
  } else if (id.startsWith('JOIN_SYNC') || id.includes('JOIN_SYNC')) {
    primaryAction = { label: 'Open Session activity', tab: 'session' };
  } else if (id.startsWith('CLIENT_ON_SERVER') || id.includes('CLIENT_ON_SERVER')) {
    const modId = str(row.mod_id) || id.replace(/^CLIENT_ON_SERVER:/i, '');
    primaryAction = {
      label: 'Open Mods (Client)',
      tab: 'mods',
      params: { view: 'overview', filter: 'client', ...(modId ? { mod: modId } : {}) },
    };
