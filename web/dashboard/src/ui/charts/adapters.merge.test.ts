...
import { mergeStableTimeSeriesRows, rowsVisuallyEqual } from './adapters.ts';
...
describe('rowsVisuallyEqual', () => {
  it('treats host_cpu changes as unequal even when vitals match', () => {
    const a = [{ date: new Date(1000), tps: 20, mspt: 10, heap_mb: 100, players: 1, host_cpu: 40 }];
    const b = [{ date: new Date(1000), tps: 20, mspt: 10, heap_mb: 100, players: 1, host_cpu: 55 }];
    assert.equal(rowsVisuallyEqual(a, b), false);
  });

  it('returns true when timestamps and series keys match', () => {
    const a = [{ date: new Date(1000), tps: 20, host_cpu: 40, net_rx_mbps: 1.2 }];
    const b = [{ date: new Date(1000), tps: 20, host_cpu: 40, net_rx_mbps: 1.2 }];
    assert.equal(rowsVisuallyEqual(a, b), true);
  });
});
