...
export function ChartFrame({
  ...
  title: string;
  /** @deprecated Layer badge moved to section-level; kept for call-site compat. */
  layer?: string;
  children: ReactNode;
  className?: string;
  actions?: ReactNode;
  ...
  const hideChart = Boolean(loading || empty);
  return (
    <div className={cn('wt-plate p-4', className)}>
      <div className="mb-3 flex items-center justify-between gap-3">
        <h3 className="text-sm font-semibold tracking-tight">{title}</h3>
        {actions}
      </div>
      {loading ? <div className="h-40 animate-pulse rounded-[var(--radius-wt)] bg-wt-bg3" /> : null}
      {!loading && error ? <ErrorState title="Chart error">{error}</ErrorState> : null}
      {!loading && !error && empty ? (
        <EmptyState title="No samples yet">Waiting for fixture data.</EmptyState>
      ) : null}
      {/* Keep the chart tree mounted so ParentSize / Y-lock survive brief gaps.
          Hide with CSS instead of unmounting (except hard errors). */}
      {!error ? (
        <div className={hideChart ? 'hidden' : undefined} aria-hidden={hideChart || undefined}>
          {children}
        </div>
      ) : null}
    </div>
...
export function GaugeChart({
  ...
  const r = size * (54 / 140);
  const stroke = Math.max(8, size * (12 / 140));
  const c = 2 * Math.PI * r;
  const dash = c * 0.75;
  const cx = size / 2;
  const cy = size / 2;
      ...
      <div className="absolute inset-0 grid place-items-center text-center">
        <div>
          <div
            ...
            style={{ fontSize: size < 110 ? '1.15rem' : '1.5rem' }}
          >
            {value.toFixed(0)}
    ...
    <div>
      ...
      <div className="h-2.5 overflow-hidden rounded-full bg-wt-bg3">
        <div
          ...
          style={{ transform: `scaleX(${Math.min(1, Math.max(0, value / 100))})`, background: color }}
        />
      </div>
    </div>
  );
    ...
    <div className="flex h-24 items-end gap-1">
      {hours.map((h, i) => (
        <div key={i} className="group relative flex-1">
          <div
            className="w-full rounded-t-sm origin-bottom transition-transform duration-300"
            style={{
              transform: `scaleY(${h / max})`,
              height: '100%',
              background: color,
...
export function PieChart({
  ...
  size?: number;
  className?: string;
  /** Compact layout for cards — packs legend beside the pie without stretching values to the far edge. */
  dense?: boolean;
}) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'min-w-0',
          dense ? 'flex min-w-[8.5rem] flex-1 flex-col justify-center space-y-1 text-[0.78rem]' : 'flex-1 space-y-1.5 text-sm',
        )}
      >
        {segments.map((s, i) => {
          const isActive = hoverIndex === i;
          ...
          return (
            <div
              ...
              tabIndex={0}
              className={cn(
                'flex cursor-pointer items-center rounded-md transition-opacity',
                dense ? 'gap-1.5 px-0.5 py-0.5' : 'gap-2 px-1.5 py-0.5',
                dimmed && 'opacity-45',
            ...
            >
              ...
              {dense ? (
                <>
                  ...
                  <span className="shrink-0 font-mono tabular-nums text-wt-text-low leading-tight">
