...
const Labels = {
  ...
  backupPillLabel(backup) {
    return Labels.backupPillLabelCombined(backup, null);
  },

  backupPillLabelCombined(backup, external, mode) {
    const ext = external?.configured ? external : null;
    const m = mode || Labels.backupModeFromParts(backup, ext);
    if (m === 'external_only' && ext) {
      if (ext.status === 'success' && !ext.stale) {
        const src = ext.source ? ` (${ext.source})` : '';
        return `Backup: OK${src}`;
      }
      if (ext.status === 'running') return 'Backup: Running';
      if (ext.stale || ext.status === 'stale') return 'Backup: Stale';
      if (ext.status === 'missing') return 'Backup: No heartbeat';
      return 'Backup: External';
    }
    if (!backup) return ext?.configured ? Labels.backupPillLabelCombined(null, ext, 'external_only') : 'Backup: Unknown';
    if (backup.status === 'success') {
      const src = ext?.source && m === 'hybrid' ? ` (${ext.source})` : '';
      return `Backup: OK${src}`;
    }
    if (backup.status === 'not_found') {
      if (ext && (ext.status === 'success' || ext.status === 'running') && !ext.stale) {
        return Labels.backupPillLabelCombined(null, ext, 'external_only');
      }
      return 'Backup: Not Found';
    }
    if (backup.status === 'unconfigured') {
      if (ext?.configured) return Labels.backupPillLabelCombined(null, ext, 'external_only');
  ...
  backupModeFromParts(backup, external) {
    const local = backup && backup.status !== 'unconfigured';
    const ext = external?.configured;
  ...
  backupWizardCopy: {
    ...
    startHereBody: 'Tell Watchtower where your backups live. If your host keeps backups on a panel (bloom, Crafty, Pterodactyl, S3, etc.) and not in a folder on this server, use the section below — it takes about 2 minutes.',
    stepWhere: 'Where your backups are',
    stepConnect: 'Connect your panel',
    phantomBanner: 'Watchtower is waiting for a backup signal you may not need. Choose <strong>On this server only</strong> below to turn that off.',
    phantomBannerTitle: 'Not using panel backups?',
    sectionWhereTitle: 'Where are your backups?',
    sectionWhereLead: 'Pick the one that sounds like you. You can change this anytime.',
    sectionConnectTitle: 'Connect your panel',
    sectionConnectLead: 'Follow the steps below. When you are done, click <strong>Test it worked</strong>.',
    wizardTitle: 'Panel backups',
    wizardIntro: 'For backups stored on your hosting panel or in the cloud — not in a folder this Minecraft server can open.',
    localSectionSummary: 'Backups in a folder on this server',
    localSectionTitle: 'Local backup folders',
    localSectionLead: 'Use this when your host saves backup files into a folder this server can read (for example Crafty mounting a backups folder).',
    offlineHint: 'Connect to the embedded dashboard on a live server to save backup settings.',
    sourceCards: [
      {
        id: 'local_only',
        title: 'On this server only',
        desc: 'Backup files (.zip or .tar.gz) are in a folder this server can read.',
        example: 'e.g. Crafty mounted a backups/ folder into the server',
      },
      {
        id: 'external_only',
        title: 'On my panel only',
        desc: 'Backups stay on your hosting panel or cloud — they never show up as files here.',
        example: 'e.g. bloom, S3, or panel-only backups',
      },
      {
        id: 'hybrid',
        title: 'I use both',
        desc: 'You have local backup files and panel or cloud backups too.',
        example: 'e.g. local zip files plus cloud copies',
      },
    ],
    recipeCopyTitle: 'Copy the command',
    recipeCopyDesc: 'Click the button — you will paste this into your panel after each backup.',
    recipeOpenTitle: 'Open your panel',
    recipePasteTitle: 'Paste it after each backup',
    recipeTestTitle: 'Come back here and test',
    recipeTestDesc: 'Click the button below to make sure Watchtower heard you.',
    setupCodeLabel: 'Your setup code',
    setupCodeHint: 'Shown once — save it if you need to edit the panel command later.',
    generateCode: 'Generate setup code',
    generateCodeHint: 'Generate a setup code to fill in the command below.',
    commandFor: (name) => `Copy-paste command for ${name}`,
    copyCommand: 'Copy command',
    testButton: 'Test it worked',
    regenerateCode: 'New setup code',
    regenerateConfirm: 'Create a new setup code? You will need to update the command in your panel.',
    successBanner: 'All set — Watchtower received your test. Real backups will show OK on the Backups tab.',
    checklistTracking: 'Panel backup tracking turned on',
    checklistCode: 'Setup code ready',
    checklistMarker: 'Status file path set',
    checklistTest: 'Test signal received',
    otherOptionsSummary: 'Other options',
    notifyMarker: 'My backup script writes a file instead',
    notifyBoth: 'Use copy-paste command and status file together',
    suppressLocal: "Don't warn when local backup folders are empty",
    markerPathLabel: 'Status file path (on this server)',
    markerPathHint: 'Your backup script should write this file after each backup:',
    copyJsonExample: 'Copy example file contents',
    saveOtherOptions: 'Save options',
    copiedToast: 'Copied to clipboard',
    codeGeneratedToast: 'Setup code ready — copy the command below',
    newCodeToast: 'New setup code created',
    testRequiresDashboard: 'Testing requires the embedded dashboard on a live server.',
    backupsTabSetupHint: 'Set up in Settings → Backups (about 2 minutes)',
    setupBackupsTitle: 'Backups',
    setupBackupsLead: 'Watchtower scans this server for backup archives automatically. You can fine-tune tracking later in Settings.',
    setupBackupsScanning: 'Looking for backup files on this server…',
    setupBackupsFoundTitle: 'Backups found',
    setupBackupsPanelFoundTitle: 'Panel backup detected',
    setupBackupsStaleTitle: 'Backups found (getting old)',
    setupBackupsCheckedTitle: 'Backup folders checked',
    setupBackupsNotFoundTitle: 'No backups found yet',
    setupBackupsNotFoundLead: 'That is normal if your host keeps backups on a panel or in a folder this server cannot read.',
    setupBackupsSettingsHint: 'Set up local folders or panel tracking anytime in Settings → Backups (about 2 minutes).',
    setupBackupsRescan: 'Scan again',
    setupBackupsScanError: 'Scan could not finish — you can still continue and set up backups later.',
    setupBackupsOpenSettings: 'Open backup settings',
  },

  backupExternalSummary(external) {
    if (!external?.configured) return 'Panel backups not set up yet.';
    const src = external.source ? ` (${external.source})` : '';
    if (external.status === 'success' && !external.stale) {
      const age = external.age_hours != null ? `${Math.round(external.age_hours)}h ago` : 'recently';
      return `Panel backup OK — last signal ${age}${src}.`;
    }
    if (external.status === 'running') {
      return `Panel backup in progress${src}${external.detail ? `: ${external.detail}` : ''}.`;
    }
    if (external.stale || external.status === 'stale') {
      return `No recent backup signal${src} — check that your panel still runs the command after each backup.`;
    }
    if (external.status === 'missing') {
      return 'Waiting for the first backup signal — paste the command into your panel or run a test.';
    }
    if (external.status === 'failed') {
      return external.detail || 'Your panel reported a backup problem.';
    }
    return 'Panel backup status unknown.';
  },

  backupExternalFixHints(external) {
    if (!external?.configured) {
      return [
        'Open Settings → Backups (about 2 minutes).',
        'Choose where your backups live, then copy the command into your panel.',
        'Click Test it worked when you are done.',
      ];
    }
    if (external.status === 'missing') {
      return [
        'Open Settings → Backups and click Test it worked.',
        'Paste the copied command into your panel so it runs after each backup.',
        'Or, under Other options, set up a status file if your script writes one.',
      ];
    }
    if (external.stale || external.status === 'stale') {
      return [
        'Check that scheduled backups still run on your panel.',
        'Open Settings → Backups to review your copy-paste command or status file path.',
      ];
    }
    return [];
  },

  backupExternalPanelGuide(panel, panelDisplayName) {
    const p = (panel || '').toLowerCase();
    const name = panelDisplayName || 'your panel';
    if (p === 'crafty') {
      return [
        `Log into ${name} and open your Minecraft server.`,
        'Go to Schedules (or your backup task) and open the task that runs backups.',
        'Look for “Run command after backup” or “Post-backup action” and paste the copied command there.',
        'Save the schedule, run a backup once, and check the Backups tab shows OK.',
      ];
    }
    if (p === 'pterodactyl') {
      return [
        `Log into ${name} and open your server.`,
        'Go to Schedules → create or edit a schedule that includes your backup task.',
        'Add a new task after the backup step: choose “Send command” and paste the copied command.',
        'Save, run a backup once, and check the Backups tab shows OK.',
      ];
    }
    if (p === 'bloom') {
      return [
        `Log into ${name} and open your server.`,
        'Open Backups or Tasks and find where you can run something after a backup finishes.',
        'Paste the copied command into that “after backup” or custom script field.',
        'Save, run a backup once, and check the Backups tab shows OK.',
      ];
    }
    if (p === 'discopanel' || p === 'docker') {
      return [
        `Log into ${name} and open your server.`,
        'Find scheduled tasks, cron jobs, or post-backup hooks in your panel settings.',
        'Paste the copied command so it runs right after each backup completes.',
        'Save, run a backup once, and check the Backups tab shows OK.',
      ];
    }
    return [
      `Log into ${name}.`,
      'Find where you set up scheduled backups or tasks that run after a backup.',
      'Paste the copied command into the “run after backup” or custom command field.',
      'Save, run a backup once, and check the Backups tab shows OK.',
    ];
  },

  backupExternalSaveSuccess(trackingMode) {
    if (trackingMode === 'off') return 'Saved — only folder backups on this server will be tracked.';
    return 'Saved. Copy the command below into your panel.';
  },

  backupExternalTestSuccess() {
    return 'It worked — Watchtower got your test signal.';
  },
  ...
  backupPillTone(backup, external, mode) {
    const label = Labels.backupPillLabelCombined(backup, external, mode);
    if (label.includes('OK') || label.includes('Running')) return 'ok';
    if (label.includes('Stale')) return 'warn';
    if (label.includes('Not Found') || label.includes('No heartbeat')) return 'down';
    if (label.includes('Unconfigured')) return 'warn';
    return 'warn';
  },
